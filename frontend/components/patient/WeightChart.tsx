...
export default function WeightChart({ logs, startWeight }: WeightChartProps) {
  ...
  const chartData = visibleLogs.map((log, index) => ({
    week: `Wk ${index + 1}`,
  ...
  const currentWeight = visibleLogs.length > 0 ? chartData[chartData.length - 1].weight : startWeight
  const initialWeight = startWeight
  const totalChange = visibleLogs.length > 0 ? (initialWeight - currentWeight) : 0
