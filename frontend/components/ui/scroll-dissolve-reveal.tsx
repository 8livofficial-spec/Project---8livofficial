...
export default function ScrollDissolveReveal({
  ...
  return (
    <div ref={sectionRef} className="relative w-full bg-white border-b border-[#00A884]/15 overflow-hidden">
      ...
      <div
        ref={pinRef}
        className="w-full min-h-screen flex flex-col justify-between py-6 sm:py-10 lg:py-12 px-3 sm:px-6 lg:px-8 max-w-7xl mx-auto relative z-10 overflow-hidden"
      >
        {/* Top Header Section */}
        <div className="text-center max-w-3xl mx-auto mb-3 sm:mb-6 px-2">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[#00A884]/10 border border-[#00A884]/20 mb-2 sm:mb-3 shadow-xs max-w-full">
            <Sparkles className="w-3.5 h-3.5 text-[#00A884] shrink-0" />
            <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.22em] text-[#0F766E] font-sora truncate">
              {eyebrow}
            </span>
          </div>
        ...
        <div className="w-full mb-3 sm:mb-6 max-w-full overflow-hidden">
          <div
            ref={tabListRef}
            className="flex items-center justify-start lg:justify-center gap-1.5 sm:gap-3 overflow-x-auto no-scrollbar py-1 px-2 sm:px-0 snap-x max-w-full"
          >
            {items.map((item, idx) => {
        ...
        <div
          ...
          className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-8 lg:gap-12 items-center max-w-6xl mx-auto w-full my-auto px-3 sm:px-4 overflow-hidden"
        >
          ...
          <div className="lg:col-span-6 relative w-full h-[200px] xs:h-[230px] sm:h-[300px] md:h-[360px] lg:h-[400px] rounded-2xl sm:rounded-3xl overflow-hidden shadow-lg sm:shadow-xl border border-slate-200 bg-slate-900 group">
            <AnimatePresence mode="wait" initial={false}>
              ...
              >
                ...
                <div className="absolute top-3 left-3 sm:top-5 sm:left-5 px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-full bg-[#0F172A]/80 backdrop-blur-md border border-white/20 text-white text-[10px] sm:text-xs font-bold font-sora shadow-md flex items-center gap-1.5 max-w-[calc(100%-24px)]">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#2DD4BF] animate-pulse shrink-0" />
                  <span className="truncate">Stage {currentItem.step} of 0{items.length}</span>
                </div>

                {/* Floating Bottom Tag */}
                <div className="absolute bottom-3 left-3 sm:bottom-5 sm:left-5 px-3 py-1 sm:px-4 sm:py-2 rounded-full bg-white/95 backdrop-blur-md border border-[#00A884]/30 text-[#00A884] text-[11px] sm:text-xs font-bold font-sora shadow-lg flex items-center gap-1.5 sm:gap-2 max-w-[calc(100%-24px)]">
                  <span className="w-2 h-2 rounded-full bg-[#00A884] shrink-0" />
                  <span className="truncate">{currentItem.cardTag}</span>
                </div>
              </motion.div>
            </AnimatePresence>
          ...
          <div className="lg:col-span-6 flex flex-col justify-center min-h-[190px] sm:min-h-[220px] min-w-0 w-full overflow-hidden">
            <AnimatePresence mode="wait" initial={false}>
              <motion.div
                key={activeStep}
                ...
                className="space-y-2.5 sm:space-y-4 min-w-0 w-full"
              >
                <div className="inline-flex items-center gap-2 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-[#00A884]/10 text-[#0F766E] border border-[#00A884]/20 text-[11px] sm:text-xs font-bold font-sora uppercase tracking-wide max-w-full">
                  <span>Stage {currentItem.step}</span>
                  <span>•</span>
                  <span className="truncate">{currentItem.badge}</span>
                </div>

                <h3 className="font-sora text-lg sm:text-2xl lg:text-3xl font-extrabold text-[#0F172A] leading-snug tracking-tight break-words">
                  {currentItem.title}
                </h3>

                <p className="text-xs sm:text-sm md:text-base text-[#475569] font-light leading-relaxed break-words">
                  {currentItem.description}
                </p>

                {/* Highlights Checklist */}
                <div className="pt-1 sm:pt-3 space-y-1.5 sm:space-y-2.5 min-w-0 w-full">
                  {currentItem.highlights.map((highlight, hIdx) => (
                    <div
                      key={hIdx}
                      className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm text-[#0F172A] font-medium font-sora bg-slate-50 sm:bg-transparent p-1.5 sm:p-0 rounded-xl border sm:border-0 border-slate-100 min-w-0 w-full"
                    >
                      ...
                      <span className="min-w-0 flex-1 break-words leading-snug">{highlight}</span>
                    </div>
                  ))}
                </div>
              </motion.div>
