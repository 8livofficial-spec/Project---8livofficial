...
export async function POST(request: Request) {
  try {
    ...
    // ATOMIC PAYOUT RESERVATION (ALL ROLES UNIFIED)
    // Executes atomic PostgreSQL function request_provider_payout_v3 with SELECT ... FOR UPDATE row-locking
    const { data: rpcData, error: rpcError } = await supabaseAdmin.rpc('request_provider_payout_v3', {
    ...
    if (rpcError) {
      // Fallback to V2 RPC if V3 RPC name is not yet migrated in database
      const { data: v2Data, error: v2Error } = await supabaseAdmin.rpc('request_provider_payout', {
        p_provider_id: provider.user.id,
        p_amount: amount,
        p_idempotency_key: idempotencyKey,
        p_initiated_by: provider.user.id,
      ...
      if (v2Error) {
        return NextResponse.json({ error: v2Error.message }, { status: 400 })
      }
      return NextResponse.json({ payout: v2Data, success: true }, { status: 201 })
    }

    return NextResponse.json({ payout: rpcData, success: true }, { status: 201 })
  } catch (error: unknown) {
